  ...
  async (req, res) => {
    try {
      ...
      const exam = new Exam({
        ...
        accessCode: crypto.randomBytes(4)
          .toString('hex')
          .toUpperCase()
      });
    ...
    try {
      ...
      if (!exam) {
        return res.status(404).json({
          message: 'Invalid access code or exam not published'
        });
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      console.log('Stats requested for exam:', req.params.id);
      ...
      if (!exam) {
        console.log('Exam not found or not owned by teacher');
        return res.status(404).json({
      ...
      if (attempts.length > 0) {
        ...
        stats.averageScore = scores.reduce(
          (a, b) => a + b, 0
        ) / scores.length;
